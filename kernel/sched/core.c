...
void check_preempt_curr(struct rq *rq, struct task_struct *p, int flags)
{
	...
	if (p->sched_class == selected->sched_class)
		selected->sched_class->check_preempt_curr(rq, p, flags);
	...
	__releases(p->pi_lock)
{
	 ...
	 * migration doesn't send it to a invalid cpu
	 */
...
static void do_activate_blocked_waiter(struct rq *target_rq, struct task_struct *pp, int en_flags)
{
	unsigned long flags;
	unsigned int state;
	struct rq_flags rf;
	...
	if (READ_ONCE(pp->on_rq)) {
		 ...
		 * all of the tasks after pp in the blocked_head list,
		 * so we're done here.
		 */
		goto out;
	}
				     ...
				     int wake_flags)
{
	...
	while (!list_empty(&bal_head)) {
		...
		while (!list_empty(&tmp_head)) {
			...
			pp = list_first_entry(&tmp_head,
					      struct task_struct,
					      blocked_node);
			WARN_ON(pp == owner);
			if (pp->sleeping_owner == owner) {
			...
			do_activate_blocked_waiter(target_rq, pp, en_flags);
			trace_sched_pe_activate_blocked_entity(owner, pp);

			raw_spin_lock_irqsave(&pp->blocked_lock, flags);
			if (list_empty(&pp->blocked_activation_node)) {
				get_task_struct(pp);
				list_add_tail(&pp->blocked_activation_node, &bal_head);
			}
			raw_spin_unlock_irqrestore(&pp->blocked_lock, flags);
...
void move_queued_task_locked(struct rq *rq, struct rq *dst_rq, struct task_struct *task)
{
	...
	WARN_ON(!task_queued_on_rq(rq, task));
			       ...
			       struct task_struct *p, int target_cpu)
{
	 ...
	 * Put rq_selected, and set rq->curr as rq_selected and set_next_task,
	 ...
	 * rq_set_selected(rq->idle) and set_next_task(rq->idle), before
	 * picking again.
	 */
