				   ...
				   struct task_struct *next)
{
	...
	if (!owner->on_rq) {
		 ...
		 */
		smp_mb();
		list_add(&next->blocked_entry, &owner->blocked_entry);
	}
}
...
pick_again:
	...
	if (next->blocked_on && next->blocked_on_waking) {
		if (!cpumask_test_cpu(cpu_of(rq), next->cpus_ptr)) {
			int cpu = select_task_rq(next, next->wake_cpu, WF_TTWU);

			put_prev_task(rq, rq_selected(rq));
			proxy_migrate_task(rq, next, &rf, next, cpu, false);
