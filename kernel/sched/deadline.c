...
static void update_curr_dl(struct rq *rq)
{
	...
	trace_sched_stat_runtime(selected, delta_exec, 0);
...
throttle:
	if (dl_runtime_exceeded(dl_se) || dl_se->dl_yielded) {
		...
		__dequeue_task_dl(rq, selected, 0);
		if (unlikely(is_dl_boosted(dl_se) || !start_dl_timer(selected)))
			enqueue_task_dl(rq, selected, ENQUEUE_REPLENISH);

		if (!is_leftmost(selected, &rq->dl))
			resched_curr(rq);
	}
